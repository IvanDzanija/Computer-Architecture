...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < platforms_n; i++) {
        ...
        CL_CHECK(clGetPlatformInfo(platforms[i], CL_PLATFORM_VERSION, 10240,
                                   buffer, NULL));
        printf("  VERSION = %s\n", buffer);
        CL_CHECK(clGetPlatformInfo(platforms[i], CL_PLATFORM_NAME, 10240,
                                   buffer, NULL));
        printf("  NAME = %s\n", buffer);
    ...
    // CL_CHECK(clGetDeviceIDs(NULL, CL_DEVICE_TYPE_ALL, 100, devices,
    // &devices_n));
    CL_CHECK(clGetDeviceIDs(platforms[index_gpu_platform], CL_DEVICE_TYPE_GPU,
                            sizeof(devices) / sizeof(devices[0]), devices,
                            &devices_n));
    ...
    for (size_t i = 0; i < devices_n; i++) {
        ...
        cl_ulong buf_ulong;
        printf("  -- %d --\n", i);
        CL_CHECK(clGetDeviceInfo(devices[i], CL_DEVICE_NAME, sizeof(buffer),
                                 buffer, NULL));
        printf("  DEVICE_NAME = %s\n", buffer);
        CL_CHECK(clGetDeviceInfo(devices[i], CL_DEVICE_VENDOR, sizeof(buffer),
                                 buffer, NULL));
        printf("  DEVICE_VENDOR = %s\n", buffer);
        CL_CHECK(clGetDeviceInfo(devices[i], CL_DEVICE_VERSION, sizeof(buffer),
                                 buffer, NULL));
        printf("  DEVICE_VERSION = %s\n", buffer);
        CL_CHECK(clGetDeviceInfo(devices[i], CL_DRIVER_VERSION, sizeof(buffer),
                                 buffer, NULL));
        printf("  DRIVER_VERSION = %s\n", buffer);
        CL_CHECK(clGetDeviceInfo(devices[i], CL_DEVICE_MAX_COMPUTE_UNITS,
                                 sizeof(buf_uint), &buf_uint, NULL));
        printf("  DEVICE_MAX_COMPUTE_UNITS = %u\n", (unsigned int)buf_uint);
        CL_CHECK(clGetDeviceInfo(devices[i], CL_DEVICE_MAX_CLOCK_FREQUENCY,
                                 sizeof(buf_uint), &buf_uint, NULL));
        printf("  DEVICE_MAX_CLOCK_FREQUENCY = %u\n", (unsigned int)buf_uint);
        CL_CHECK(clGetDeviceInfo(devices[i], CL_DEVICE_GLOBAL_MEM_SIZE,
    ...
    const char *program_source[] = {"__kernel void simple_demo(__global int "
                                    "*src, __global int *dst, int factor)\n",
                                    "{\n", "	int i = get_global_id(0);\n",
                                    "	dst[i] = src[i] * factor;\n", "}\n"};
    cl_program program = (cl_program)CL_CHECK_ERR(clCreateProgramWithSource(
        context, sizeof(program_source) / sizeof(*program_source),
        program_source, NULL, &_err));
    ...
    cl_command_queue queue = (cl_command_queue)CL_CHECK_ERR(
        clCreateCommandQueue(context, devices[0], 0, &_err));
    cl_mem input_buffer = (cl_mem)CL_CHECK_ERR(clCreateBuffer(
        context, CL_MEM_READ_ONLY, sizeof(int) * NUM_DATA, NULL, &_err));
    cl_mem output_buffer = (cl_mem)CL_CHECK_ERR(clCreateBuffer(
        context, CL_MEM_WRITE_ONLY, sizeof(int) * NUM_DATA, NULL, &_err));
    CL_CHECK(clEnqueueWriteBuffer(queue, input_buffer, CL_TRUE, 0,
                                  sizeof(int) * NUM_DATA, buf_in, 0, NULL,
                                  NULL));
    int factor = 2;
    cl_kernel kernel =
        (cl_kernel)CL_CHECK_ERR(clCreateKernel(program, "simple_demo", &_err));
